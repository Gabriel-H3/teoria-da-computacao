**40 questões objetivas** (múltipla escolha) sobre o **Histórico da Criptografia e Algoritmos Simétricos e Assimétricos**
...
**B) Uma cifra de substituição monoalfabética com deslocamento fixo (X)**
...
**B) Cifra de substituição polialfabética com rotores (X)**
...
**B) Democratizou o uso da criptografia assimétrica para o público em geral, protegendo e-mails e arquivos (X)**
...
**C) DES (Data Encryption Standard) (X)**
...
**D) AES (Advanced Encryption Standard) (X)**
...
**B) O DES tinha chave de 56 bits, tornando-se vulnerável a ataques de força bruta (X)**
...
**B) Utilizar a mesma chave tanto para cifrar quanto para decifrar a mensagem (X)**
...
**B) Maior eficiência computacional e velocidade para grandes volumes de dados (X)**
...
**B) Cifras de fluxo (stream ciphers) (X)**
...
**B) Bloco 64 bits, chave 56 bits (X)**
...
**B) O pequeno tamanho de bloco de 64 bits (X)**
...
**B) Em protocolos IPSec e OpenPGP, sendo aprovado por diversos padrões internacionais (X)**
...
**B) Manterá segurança equivalente a uma chave de 128 bits devido ao algoritmo de Grover (X)**
...
**B) Utiliza um par de chaves matematicamente relacionadas: uma pública e uma privada (X)**
...
**C) Permitir a troca segura de chaves entre duas partes em um canal inseguro (X)**
...
**B) É vulnerável a ataques de homem-no-meio (Man-in-the-Middle) (X)**
...
**C) Fatoração de números inteiros grandes (X)**
...
**B) Porque é 10 a 1.000 vezes mais lento que algoritmos simétricos (X)**
...
**B) Utilizar o RSA apenas para cifrar uma chave de sessão simétrica (como AES), que então cifra o conteúdo (X)**
...
**B) Oferece o mesmo nível de segurança com chaves significativamente menores e menor custo computacional (X)**
...
**B) ECDSA (Elliptic Curve Digital Signature Algorithm) (X)**
...
**B) Desenvolvimento de computadores quânticos em larga escala com o algoritmo de Shor (X)**
